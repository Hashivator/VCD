...
try:
    from PySide6.QtCore import (
        ...
        QSettings, QUrl, QPointF, QPropertyAnimation, QEasingCurve,
        QStringListModel,
    )
    from PySide6.QtGui import (
        QColor, QPainter, QLinearGradient, QRadialGradient, QDesktopServices,
        QPen, QPainterPath, QPixmap, QIcon, QKeySequence, QShortcut, QAction,
    )
    from PySide6.QtWidgets import (
        QApplication, QWidget, QLabel, QLineEdit, QPushButton, QComboBox,
        QSpinBox, QSlider, QCheckBox, QPlainTextEdit, QFrame, QVBoxLayout,
        QHBoxLayout, QGridLayout, QButtonGroup, QGraphicsDropShadowEffect,
        QFileDialog, QMessageBox, QToolButton, QScrollArea, QProgressBar,
        QTabWidget, QSplitter, QTableWidget, QTableWidgetItem,
        QListWidget, QListWidgetItem, QSystemTrayIcon, QMenu,
        QCompleter, QAbstractItemView, QSizePolicy, QDialog, QDialogButtonBox,
    )
except ImportError:
...
LEVEL_COLOR = {
    "INFO": SKY, "SUCCESS": GREEN, "STEP": VIO,
    "WARN": AMBER, "ERROR": RED, "DEBUG": DGREY, "LOG": TEAL,
}
LEVEL_ICON = {
    "INFO": "›", "SUCCESS": "✓", "STEP": "◆",
    "WARN": "!", "ERROR": "✗", "DEBUG": "·", "LOG": "·",
}
...
# ---------------------------------------------------------------------------
PRESETS = {
    "Ultra (1080p)":   dict(w=1920, h=1080, fps=30, crf=20, vp="slow",
                            ab="192k", pad=2000, note="Best quality — slow encode, biggest file."),
    "High (720p - high)":     dict(w=1280, h=720,  fps=30, crf=23, vp="medium",
                            ab="128k", pad=2000, note="Great quality, reasonable size."),
    "Balanced (720p - normal)": dict(w=1280, h=720,  fps=30, crf=28, vp="veryfast",
                            ab="96k",  pad=2000, note="Fast encode, decent quality. Good for lectures."),
    "Compact (480p)":  dict(w=854,  h=480,  fps=24, crf=32, vp="veryfast",
                            ab="64k",  pad=2000, note="Smallest file, fastest to make."),
    "Custom":          None,
}
RESOLUTIONS     = ["1920x1080","1600x900","1280x720","960x540","854x480","640x360"]
VPRESETS        = ["ultrafast","superfast","veryfast","faster","fast","medium","slow","slower"]
...
_QUEUE_CLR = {"pending":"#7aacca","running":"#38bdf8","done":"#34d399","failed":"#f87171"}
...
class StarField(QWidget):
    def __init__(self, parent=None, n=160):
        ...
        self.stars = [dict(x=rnd.random(),y=rnd.random(),r=rnd.uniform(.5,1.9),
            base=rnd.uniform(25,155),amp=rnd.uniform(18,75),sp=rnd.uniform(.4,2.0),
            ph=rnd.uniform(0,6.28),drift=rnd.uniform(.003,.015)) for _ in range(n)]
        self.big = [dict(x=rnd.random(),y=rnd.random(),r=rnd.uniform(1.4,2.5),
            col=rnd.choice([(56,189,248),(167,139,250),(99,102,241)]),
            sp=rnd.uniform(.3,1.0),ph=rnd.uniform(0,6.28),drift=rnd.uniform(.002,.008))
            for _ in range(9)]
        self._t = 0.0
    ...
    def paintEvent(self, _ev):
        ...
        for cx, cy, col in ((0.15,0.20,(56,189,248)),(0.88,0.82,(167,139,250)),(0.68,0.08,(99,102,241))):
            r,g,b = col
            rg = QRadialGradient(cx*w, cy*h, 0.48*max(w,h))
            rg.setColorAt(0.0, QColor(r,g,b,20)); rg.setColorAt(1.0, QColor(r,g,b,0))
        ...
        for s in self.big:
            ...
            glow.setColorAt(0.0, QColor(r,g,b,int(a*.45))); glow.setColorAt(1.0, QColor(r,g,b,0))
            p.setBrush(glow); p.drawEllipse(QPointF(x,y),11,11)
            p.setBrush(QColor(r,g,b,a)); p.drawEllipse(QPointF(x,y),s["r"],s["r"])
        p.end()
...
class SpeedGraph(QWidget):
    ...
    def paintEvent(self, _ev):
        ...
        n  = len(self._pts)
        pts = [QPointF(w*i/(n-1),(h-4)-(h-8)*self._pts[i]/mx) for i in range(n)]
        path = QPainterPath()
...
class Worker(QObject):
    ...
    def cancel(self):
        self._cancel = True
        # kill ffmpeg immediately if it's running — don't wait for it to finish
        try:
            proc = getattr(core, "_current_proc", None)
            if proc and proc.poll() is None:   # poll() is None = still running
                proc.terminate()
        except Exception:
            pass

        # stop download — closing the socket interrupts iter_content immediately
        try:
            resp = getattr(core, "_current_response", None)
            if resp is not None:
                resp.close()
        except Exception:
            pass
    ...
    def run(self):
        ...
        router = _StreamRouter(
            lambda lvl, msg: self.sig_log.emit(lvl, msg),
            lambda mode, pct: self.sig_progress.emit(mode, pct),
            lambda spd: self.sig_speed.emit(spd),
            lambda eta: self.sig_eta.emit(eta),
            lambda dl, tot: self.sig_bytes.emit(dl, tot),
        )
        try:
            # kill the interactive input() fallback — we handle auth in the GUI
            try:
                core._login_via_manual_cookie = lambda *a, **k: None
            except Exception:
                pass
            sys.stdout = router
            sys.stderr = router
            ...
            tools = core.init_tools()
            self._ck()

            rid = p["rid"]
            if p["reuse"] and Path(rid).is_dir():
                self.sig_log.emit("INFO", f"Found existing folder '{rid}' — skipping download.")
                result_dir = Path(rid)
            else:
                self.sig_stage.emit("Downloading class files...")
                dl_cfg = core.DownloadConfig(verify_ssl=p["verify_ssl"])
                result_dir = core.download_and_extract(
                    p["url"], rid, dl_cfg, (p["cookie"] or None))
            self._ck()

            rcfg = core.RenderConfig(
                canvas_w=p["w"], canvas_h=p["h"], fps=p["fps"], crf=p["crf"],
                video_preset=p["vp"], audio_bitrate=p["ab"], padding_ms=p["pad"],
                gpu=p.get("gpu", "cpu"),
            )
            stage = "Building timeline..." if p["xml_only"] else "Rendering video..."
            self.sig_stage.emit(stage)
            core.process_recording(tools, str(result_dir), p["output_name"], p["xml_only"], rcfg)
            self._ck()

            out_path = (str((result_dir / "timeline.xml").resolve()) if p["xml_only"]
                        else str(Path(p["output_name"]).resolve()))
            self.sig_done.emit(True, "Done! 🎓", out_path)

        except _Cancelled:
            self.sig_done.emit(False, "Stopped.", "")
        except Exception as exc:
            if self._cancel:
                # cancel() was called — the exception is a side-effect, not a real error
                self.sig_done.emit(False, "Stopped.", "")
            else:
                friendly = ((core is not None and isinstance(exc, core.VCDError))
                            or isinstance(exc, FileNotFoundError))
                self.sig_done.emit(False, str(exc) if friendly else f"{type(exc).__name__}: {exc}", "")
        finally:
...
class MainWindow(StarField):
    ...
    def _build_header(self):
        ...
        self.sc_ff = QLabel("FFmpeg …"); self.sc_ff.setObjectName("sys")
        self.sc_fp = QLabel("FFprobe …"); self.sc_fp.setObjectName("sys")
    ...
    def _build_left(self):
        ...
        self.cookie_edit.setPlaceholderText("Cookie  (optional — only if no ?session= in URL)")
        ...
        self.res_cb = QComboBox(); self.res_cb.addItems(RESOLUTIONS)
        self.fps_sb = QSpinBox();  self.fps_sb.setRange(5, 60)
        self.crf_sl = QSlider(Qt.Horizontal); self.crf_sl.setRange(0, 51)
        self.crf_val = QLabel("28"); self.crf_val.setFixedWidth(26); self.crf_val.setAlignment(Qt.AlignCenter)
        self.ab_cb  = QComboBox(); self.ab_cb.addItems(ABITRATES)
        self.vp_cb  = QComboBox(); self.vp_cb.addItems(VPRESETS)
        self.pad_sb = QSpinBox();  self.pad_sb.setRange(0,15000); self.pad_sb.setSingleStep(250); self.pad_sb.setSuffix(" ms")
        crf_w = QWidget()
        cl = QHBoxLayout(crf_w); cl.setContentsMargins(0,0,0,0); cl.setSpacing(6)
        cl.addWidget(self.crf_sl, 1); cl.addWidget(self.crf_val)
        ag.addWidget(self._fld("Resolution"),0,0); ag.addWidget(self.res_cb,0,1)
        ag.addWidget(self._fld("FPS"),        0,2); ag.addWidget(self.fps_sb,0,3)
        ag.addWidget(self._fld("Quality CRF"),1,0); ag.addWidget(crf_w,     1,1)
        ag.addWidget(self._fld("Audio"),      1,2); ag.addWidget(self.ab_cb, 1,3)
        ag.addWidget(self._fld("Encoder"),    2,0); ag.addWidget(self.vp_cb, 2,1)
        ag.addWidget(self._fld("Tail pad"),   2,2); ag.addWidget(self.pad_sb,2,3)
        ...
        self.reuse_chk      = QCheckBox("Reuse already-downloaded files"); self.reuse_chk.setChecked(True)
        self.auto_retry_chk = QCheckBox(f"Auto-retry on failure  (up to {_MAX_RETRIES}×)"); self.auto_retry_chk.setChecked(True)
    ...
    def _build_log_tab(self):
        ...
        v = QVBoxLayout(panel); v.setContentsMargins(16,14,16,16); v.setSpacing(6)

        head = QHBoxLayout()
        head.addWidget(self._section("Live Log"))
        head.addStretch(1)
        self.clear_btn = QPushButton("Clear"); self.clear_btn.setObjectName("ghost"); self.clear_btn.setCursor(Qt.PointingHandCursor)
        self.save_btn  = QPushButton("Save");  self.save_btn.setObjectName("ghost");  self.save_btn.setCursor(Qt.PointingHandCursor)
        head.addWidget(self.clear_btn); head.addWidget(self.save_btn)
        v.addLayout(head)
        ...
        if core is None:
            self._append_log("ERROR", "vcd_core.py not found. Check the startup error dialog.")
        return panel

    def _build_history_tab(self):
        panel = QFrame(); panel.setObjectName("panel")
        v = QVBoxLayout(panel); v.setContentsMargins(16,14,16,16); v.setSpacing(8)

        head = QHBoxLayout()
        head.addWidget(self._section("Job History"))
        head.addStretch(1)
        rb = QPushButton("Refresh"); rb.setObjectName("ghost"); rb.setCursor(Qt.PointingHandCursor)
        rb.clicked.connect(self._refresh_history)
        cb = QPushButton("Clear All"); cb.setObjectName("ghost"); cb.setCursor(Qt.PointingHandCursor)
        cb.clicked.connect(self._clear_history)
        head.addWidget(rb); head.addWidget(cb)
        v.addLayout(head)

        self.hist_table = QTableWidget()
        self.hist_table.setColumnCount(7)
        self.hist_table.setHorizontalHeaderLabels(["Date","ID","Status","Size","Duration","Preset","File"])
        ...
        note = QLabel("Right-click a row for options  ·  Double-click to open file  ·  Ctrl+H to focus")
    ...
    def _build_files_tab(self):
        ...
        head.addStretch(1)
        rf = QPushButton("Refresh"); rf.setObjectName("ghost"); rf.setCursor(Qt.PointingHandCursor)
    ...
    def _wire(self):
        ...
        self.fname_edit.textEdited.connect(lambda *_: setattr(self,"_fname_touched",True))
        self.browse_btn.clicked.connect(self._browse_dir)
        self.cookie_profile_cb.currentIndexChanged.connect(self._on_cookie_profile_selected)
    ...
    def _setup_shortcuts(self):
        ...
        QShortcut(QKeySequence("Ctrl+Shift+Q"), self).activated.connect(self._queue_add_current)
    ...
    def _read_render_params(self) -> dict:
        ...
        return dict(w=w, h=h, fps=self.fps_sb.value(), crf=self.crf_sl.value(),
                    vp=self.vp_cb.currentText(), ab=self.ab_cb.currentText(),
                    pad=self.pad_sb.value(), gpu=GPU_KEYS[gpu_idx])
    ...
    def _refresh_detected(self):
        ...
        tok_txt = (f"session ✓  {tok[:8]}…" if tok else "no session token — paste cookie if needed")
    ...
    def _system_check(self):
        if core is None:
            for w in (self.sc_ff, self.sc_fp):
                w.setText("core missing"); w.setStyleSheet(f"color:{RED};")
            return
        ff = core.find_tool("ffmpeg")
        fp = core.find_tool("ffprobe")
        for w, ok, name, path in ((self.sc_ff,ff,"FFmpeg",ff),(self.sc_fp,fp,"FFprobe",fp)):
            w.setText(f"{name} {'✓' if ok else '✗'}")
            w.setStyleSheet(f"color:{GREEN if ok else RED}; font-weight:700;")
            if path:
                w.setToolTip(str(path))
        if not (ff and fp):
            self._append_log("WARN", "FFmpeg or FFprobe not found — rendering will fail.")
    ...
    def _detect_gpu_encoders(self):
        ...
        try:
            import subprocess
            r = subprocess.run([ff, "-encoders"], capture_output=True, text=True, timeout=6)
    ...
    def _build_params_from_ui(self) -> Optional[dict]:
        ...
        if not self.xml_chk.isChecked() and not fname.lower().endswith(".mp4"):
            fname += ".mp4"
        return dict(url=url, rid=rid, output_dir=out_dir, output_name=fname,
    ...
    def _queue_add_current(self):
        ...
        self._append_log("INFO", f"Queued: {params['rid']}  ({len(self._queue)} total in queue)")

    def _queue_run(self):
        if self._running:
            self._append_log("WARN", "Job running — queue will continue automatically after it finishes.")
            return
        if not self._queue:
    ...
    def _start(self):
        ...
        if not ok:
            r = QMessageBox.warning(self, "Low Disk Space",
    ...
    def _start_job(self, params: dict):
        ...
        self._append_log("INFO", f"Output → {os.path.join(params['output_dir'], params['output_name'])}")
    ...
    def _set_running(self, running: bool):
        ...
        for w in (self.url_edit, self.cookie_edit, self.dir_edit,
                  self.fname_edit, self.browse_btn, self.adv):
    ...
    def _on_progress(self, mode: str, pct: int):
        if mode == "busy":
            self.bar.setRange(0, 0); self.bar.setFormat(""); return
    ...
    def _on_done(self, ok: bool, message: str, out_path: str):
        ...
        entry = {
            ...
            "size":        size_str,
            "duration":    dur_str,
            "preset":      self._last_params.get("preset","?") if self._last_params else "?",
        ...
        if ok:
            ...
            if self._tray:
                self._tray.notify("VCD — Done!", f"{os.path.basename(out_path)} — {detail or 'finished'}")
    ...
    def _auto_retry(self) -> bool:
        ...
        if self._retry_count >= _MAX_RETRIES:
            self._append_log("WARN", f"Max retries ({_MAX_RETRIES}) reached. Giving up.")
            return False
        ...
        QTimer.singleShot(secs * 1000, lambda: (
            self.retry_lbl.setVisible(False),
    ...
    def _maybe_extract_thumb(self, video_path: str):
        ...
        try:
            ...
            r = subprocess.run(
                [ff, "-y", "-ss", f"{t:.1f}", "-i", video_path,
                 "-frames:v", "1", "-vf", "scale=260:-1", "-q:v", "4", out],
    ...
    def _hist_context_menu(self, pos):
        ...
        menu.addAction("✕  Remove from history", lambda: (self._history.remove(row), self._refresh_history()))
    ...
    def _build_file_card(self, entry: dict) -> QFrame:
        ...
        if thumb and os.path.isfile(thumb):
            px = QPixmap(thumb).scaled(80, 46, Qt.KeepAspectRatio, Qt.SmoothTransformation)
        ...
        info = QVBoxLayout()
        info.setSpacing(2)
        name_lbl = QLabel(fname); name_lbl.setObjectName("file_name")
        meta_parts = [p for p in [size, dur, date] if p]
        meta_lbl = QLabel("  ·  ".join(meta_parts)); meta_lbl.setObjectName("file_meta")
        info.addWidget(name_lbl)
        info.addWidget(meta_lbl)
        row.addLayout(info, 1)

        # action buttons
        bcol = QVBoxLayout(); bcol.setSpacing(4)
        for icon, tip, fn, obj in [
            ("▶", "Play",        lambda _=False, p=path: self._play_file(p),            "icon_btn"),
            ("📂","Open folder", lambda _=False, p=path: self._open_in_folder(p),       "icon_btn"),
            ("🗑","Delete file", lambda _=False, p=path: self._delete_output_file(p),   "danger_btn"),
        ]:
            b = QPushButton(icon); b.setObjectName(obj); b.setToolTip(tip)
            b.setFixedWidth(32); b.setCursor(Qt.PointingHandCursor)
            b.clicked.connect(fn); bcol.addWidget(b)
        row.addLayout(bcol)
        return card
    ...
    def _append_log(self, level: str, msg: str):
        ...
        frag  = (f'<span style="color:#1e3a55">[{ts}]</span> '
    ...
    def _save_log(self):
        fn, _ = QFileDialog.getSaveFileName(self, "Save log", "vcd_log.txt", "Text (*.txt)")
        if fn:
            try:
    ...
    def _about(self):
        QMessageBox.information(self, "About VCD v0.3",
    ...
    def closeEvent(self, e):
        if self._running and self.thread:
            r = QMessageBox.question(self, "Quit?",
